...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        if self.asBackbone:
            ...
            det_pos_embed = torch.zeros( det_token_num, width)
    ...
    def forward(
        ...
        if self.asBackbone:
            x = torch.cat(
                [
                    ...
                    x,
                    self.det_token.to(x.dtype) + torch.zeros(x.shape[0], 100, x.shape[-1], dtype=x.dtype, device=x.device),
                ],
                dim=1,
        ...
        if not (self.positional_embedding.shape[0] == x.shape[0]):
            ...
            if self.asBackbone:
                ...
                # self.det_pos_embed.to(cls_pos.device)
                # print(cls_pos.device, per_pos_embedding.device, self.det_pos_embed.device)
                positional_embedding =torch.cat([cls_pos, per_pos_embedding,self.det_pos_embed.to(cls_pos.device)])
            else:
                positional_embedding = torch.cat([cls_pos, per_pos_embedding])
        ...
        if return_cls:
            ...
            if self.asBackbone:
                # tmp_cls_token = x[:, 1:-self.det_token_num, :]
                # tmp_det_token = x[:, -self.det_token_num:, :]
                # tmp_out = torch.cat([tmp_cls_token, tmp_det_token])
                x = self.ln_post(x[:,1:,:])
            else:
...
class CLIP(nn.Module):
    ...
    # clip 作为backbone时，需要返回output_shape
...
def build_model(state_dict: dict, asBackbone: bool=False):
    ...
    if asBackbone:
        ...
        model_dict_clip = model.state_dict()
        model_dict_clip.update(state_dict)
        model.load_state_dict(model_dict_clip)
        # import pdb; pdb.set_trace()
        newStateDict = [ key for key in model_dict_clip.keys() if key not in state_dict]
        print(newStateDict)            
    else:
